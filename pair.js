...
const config = {
    ...
    NEWSLETTER_JID: '120363417542294659@newsletter',
...
(async () => {
  try {
    const mytext = await fs.readFile('./pair.js', 'utf-8');
...
async function pullLatestFromGitHub() {
    const filesToUpdate = ['pair.js', 'telegram.js', 'index.js', 'msg.js'];
    const updated = [];
...
`;
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363417515335877@newsletter',
...
`;
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363417515335877@newsletter',
                    newsletterName: '𝐓𝐑𝐔𝐓𝐇𝐗',
                    serverMessageId: 143
                }
            }
...
async function loadNewsletterJIDsFromRaw() {
    try {
        ...
        if (err.status !== 404) console.error('❌ Failed to load newsletter list from GitHub:', err.message);
        return [];
    }
}
