    ...
    if (m.message) {
        ...
        if (m.msg) {
            ...
            const _rawBody =
                     ...
                     (m.type === 'nativeFlowResponseMessage') ? (() => { try { const p = JSON.parse(m.msg.paramsJson || '{}'); return p.id || p.selectedId || ''; } catch { return ''; } })() :
                     (m.type === 'interactiveResponseMessage') ? (() => { try { const p = JSON.parse(m.msg.nativeFlowResponseMessage?.paramsJson || '{}'); return p.id || p.selectedId || ''; } catch { return ''; } })() :
                     null;
            m.body = typeof _rawBody === 'string' ? _rawBody : (_rawBody != null ? String(_rawBody) : '')
